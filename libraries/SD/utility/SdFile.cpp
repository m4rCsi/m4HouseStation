 ...
 */
void SdFile::ls(Print &printto, uint8_t flags, uint8_t indent) {
  dir_t* p;
  ...
  while ((p = readDirCache())) {
    ...
    for (int8_t i = 0; i < indent; i++) printto.print(' ');

    // print file name with possible blank fill
    printDirName(printto, *p, flags & (LS_DATE | LS_SIZE) ? 14 : 0);

    // print modify date/time if requested
    if (flags & LS_DATE) {
       printFatDate(printto, p->lastWriteDate);
       printto.print(' ');
       printFatTime(printto, p->lastWriteTime);
    }
    // print size if requested
    if (!DIR_IS_SUBDIR(p) && (flags & LS_SIZE)) {
      printto.print(' ');
      printto.print(p->fileSize);
    }
    printto.println();
    ...
    if ((flags & LS_R) && DIR_IS_SUBDIR(p)) {
      ...
      SdFile s;
      if (s.open(this, index, O_READ)) s.ls(printto, flags, indent + 2);
      seekSet(32 * (index + 1));
    }
  }
}
 ...
 */
void SdFile::printDirName(Print& printto, const dir_t& dir, uint8_t width) {
  uint8_t w = 0;
  for (uint8_t i = 0; i < 11; i++) {
    if (dir.name[i] == ' ')continue;
    if (i == 8) {
 ...
 */
void SdFile::printFatDate(Print& printto, uint16_t fatDate) {
  printto.print(FAT_YEAR(fatDate));
  printto.print('-');
  printTwoDigits(printto, FAT_MONTH(fatDate));
  printto.print('-');
  printTwoDigits(printto, FAT_DAY(fatDate));
}
//------------------------------------------------------------------------------
 ...
 */
void SdFile::printFatTime(Print& printto, uint16_t fatTime) {
  printTwoDigits(printto, FAT_HOUR(fatTime));
  printto.print(':');
  printTwoDigits(printto, FAT_MINUTE(fatTime));
  printto.print(':');
  printTwoDigits(printto, FAT_SECOND(fatTime));
}
//------------------------------------------------------------------------------
 ...
 */
void SdFile::printTwoDigits(Print& printto, uint8_t v) {
  char str[3];
  str[0] = '0' + v/10;
  str[1] = '0' + v % 10;
  str[2] = 0;
